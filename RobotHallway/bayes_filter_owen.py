...
class BayesFilter:
    ...
    def reset_probabilities(self, n_bins: int = 10):
        ...
        # YOUR CODE HERE
        self.probabilities = np.zeros(n_bins)
        for i , bin in enumerate(self.probabilities):
            self.probabilities[i] = 1.0 / n_bins
    ...
    def update_belief_sensor_reading(self, world_ground_truth, robot_sensor, sensor_reading):
        ...
        atDoor = False
        binSize = 1 / np.size(self.probabilities)

        newProb = np.zeros(np.size(self.probabilities))
        num = 0

        for i , bin_prob in enumerate(self.probabilities):
            #Checking if robot is at a door
            atDoor = False
            for door in world_ground_truth.doors:
                if (binSize/2) + i * binSize == door:
                    atDoor = True
                    break
            
            if atDoor:
                #At a door and sensor reads at a door
                if sensor_reading:
                    newProb[i] = robot_sensor.door_probabilities["door"]["sees door"] * bin_prob

                #At a door and sensor reads no door
                else:
                    newProb[i] = robot_sensor.door_probabilities["door"]["sees no door"] * bin_prob

            else:
                #Not at a door and sensor reads at a door
                if sensor_reading:
                    newProb[i] = robot_sensor.door_probabilities["no door"]["sees door"] * bin_prob

                #Not at a door and sensor reads no door
                else:
                    newProb[i] = robot_sensor.door_probabilities["no door"]["sees no door"] * bin_prob
            
            num += newProb[i]
        
        for i in range(np.size(newProb)):
            self.probabilities[i] = newProb[i] / num
    ...
    def update_belief_move_left(self, robot_ground_truth):
        ...
        for i , bin in enumerate(self.probabilities):
            if i == 0: #Bin is the far left one, if the robot moves left it stays in the same spot
                ...
                newProbs[i] += robot_ground_truth.move_probabilities["move_left"]["stay put"] * self.probabilities[i]
    ...
    def update_belief_move_right(self, robot_ground_truth):
        ...
        for i , bin in enumerate(self.probabilities):
            if i == 0: #Bin is the far left one, if the robot moves left it stays in the same spot
                ...
                newProbs[i] += robot_ground_truth.move_probabilities["move_right"]["stay put"] * self.probabilities[i]
